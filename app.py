...
# Initialize session state to remember checkbox selections
if "user_needs" not in st.session_state:
    st.session_state.user_needs = {step: None for step in steps.keys()}

# Questionnaire with improved checkbox styling
...
for step_name, step_info in steps.items():
    ...
    # Checkbox options for "Yes" and "No"
    col1, col2 = st.columns(2)
    with col1:
        st.session_state.user_needs[step_name] = st.checkbox(f"Yes, I need help with {step_name}", 
                                                             value=(st.session_state.user_needs[step_name] == "Yes"))
    with col2:
        st.session_state.user_needs[step_name] = "No" if not st.session_state.user_needs[step_name] else "Yes"
